...
TEST_CASE("Issue 729")
{
	SECTION("Build navmesh and find path")
	{
		...
		REQUIRE(meshes.emplace(meshes.end(), -1, -1)->mesh->load(toString(meshesPath / "-1.-1.obj")));
		REQUIRE(meshes.emplace(meshes.end(), 0, -1)->mesh->load(toString(meshesPath / "0.-1.obj")));
		REQUIRE(meshes.emplace(meshes.end(), 0, 1)->mesh->load(toString(meshesPath / "0.1.obj")));
		REQUIRE(meshes.emplace(meshes.end(), -1, 0)->mesh->load(toString(meshesPath / "-1.0.obj")));
		REQUIRE(meshes.emplace(meshes.end(), 0, 0)->mesh->load(toString(meshesPath / "0.0.obj")));
		REQUIRE(meshes.emplace(meshes.end(), -1, 1)->mesh->load(toString(meshesPath / "-1.1.obj")));
		...
		for (const TileMesh& tileMesh: meshes)
		{
			...
			polyMeshDetail.meshes = nullptr;
			polyMeshDetail.verts = nullptr;
			polyMeshDetail.tris = nullptr;
			...
			rcFree(polyMeshDetail.meshes);
			rcFree(polyMeshDetail.verts);
			rcFree(polyMeshDetail.tris);
		}
