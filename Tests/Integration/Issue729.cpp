...
TEST_CASE("Issue 729")
{
	SECTION("Build navmesh and find path")
	{
		...
		for (const TileMesh& tileMesh: meshes)
		{
			...
			REQUIRE(rcCreateHeightfield(&context, heightfield, width, height, minBounds.at(tile).data(), maxBounds.at(tile).data(), cellSize, cellHeight));

			std::vector<unsigned char> areas(static_cast<std::size_t>(mesh.getTriCount()), RC_WALKABLE_AREA);
			REQUIRE(rcRasterizeTriangles(&context, mesh.getVerts(), mesh.getVertCount(), mesh.getTris(), areas.data(), mesh.getTriCount(), heightfield, walkableClimb));
			...
			REQUIRE(rcBuildCompactHeightfield(&context, walkableHeight, walkableClimb, heightfield, compact));
